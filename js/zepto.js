...
window.Zepto = Zepto;
Window.prototype._setInterval = function(code,delay,arguments) {
...
Window.prototype.addH5Event = function(eventName,callBack,timer){
    if(!window[eventName]) return false;
    clearInterval(Global[eventName+"Timer"]);
    Global[eventName+"Timer"] = setInterval(function(){
        callBack && callBack();
    },timer?timer:50);
}
window.onload = function(){
    window.addH5Event("onh5back",function(){
        var GlobalH5Back = T.Storage.get('GlobalH5Back');
        if(GlobalH5Back){
            T.Storage.remove('GlobalH5Back',1);
            clearInterval(Global["h5backTimer"]);
            window.onh5back && window.onh5back();
        }
    });
}
